...
export class Utils {
    ...
    static async search(
        ...
        try {
            ...
            if (SOptions?.uploadDate !== null)
                Filters =
                    Array.from(
                        (await YTSR.getFilters(Filters.url!)).get(
                            "Upload date"
                        )!,
            ...
            if (SOptions?.sortBy !== null && SOptions?.sortBy !== "relevance")
                Filters =
                    Array.from(
                        ...
                        o.name.toLowerCase().includes(SOptions?.sortBy!)
                    ) ?? Filters;

            let Result = await YTSR(Filters.url!, {
                limit: Limit
            });

            let items = Result.items as Video[];

            let songs: (Song | null)[] = items
                .map((item) => {
    ...
    static async playlist(
        ...
        let SoundCloudPlaylistLink =
            this.regexList.SoundCloudPlaylist.test(Search);
        ...
        if (ApplePlaylistLink) {
            ...
            let SoundCloudResultInfo = await new SoundCloudScraperClient()
                // .getSongInfo(Search)
                .getSongInfo(
                    "https://soundcloud.com/atif-aslam-116789906/sajan-das-na-atif-aslam-momina-mustehsan-coke-studio-14"
                )
                .catch(() => null);
            ...
            let SpotifyResult: RawPlaylist = {
                ...
                author:
                    SpotifyResultData.type === "playlist"
                        ? SpotifyResultData.owner.display_name
            ...
            SpotifyResult.songs = (
                await Promise.all(
                    (SpotifyResultData.tracks?.items ?? []).map(
                        async (track: any, index: number) => {
                            if (Limit !== -1 && index >= Limit) return null;
            ...
            if (
                !YouTubeResultData ||
                Object.keys(YouTubeResultData).length === 0
            )
                throw DMPErrors.INVALID_PLAYLIST;
            ...
            let YouTubeResult: RawPlaylist = {
                ...
                author:
                    YouTubeResultData instanceof IPlaylist
                        ? YouTubeResultData.channel?.name ?? "YouTube Mix"
                        : "YouTube Mix",
                url: Search,
                songs: [],
                type: "playlist"
